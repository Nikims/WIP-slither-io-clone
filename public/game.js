...
function update() {
  ...
  for (i = 0; i < foods.length; i++) {
    if (
      areColliding(300, 300, 40, 40, foods[i].x-mypos.x+300, foods[i].x-mypos.y+300, 10, 10)
    ) {
      length++;
      foods.splice(i, 1);
      break;
...
function draw() {
  ...
  for (i = 0; i < foods.length; i++) {
    context.fillRect(foods[i].x-mypos.x+300, foods[i].y-mypos.y+300, 10, 10);
  }
  for (i = 0; i < mypos.coords.length; i++) {
    context.fillRect(mypos.coords[i].x-mypos.x+300, mypos.coords[i].y-mypos.y+300, 40, 40);
  }
